...
# DobA4
doba4_data <- read.csv("Inflow_Outflow/DobA4_Water_Level_all_corrected.csv")
...
```{r}
plot(elevation_relationship_ft, elevation_outflow_gar, type = "l")
grid()

plot(elevation_relationship_ft, elevation_outflow_mid, type = "l")
grid()
```
```{r}
...
Garner Fancy Rating Curve Plot
```{r}
library(ggplot2)

#elevation_relationship_ft <- elevation_relationship_ft / 3.28
#elevation_outflow_gar <- elevation_outflow_gar / 35.3146667
gar_rating_curve <- data.frame(elevation_relationship_ft = elevation_relationship_ft, elevation_outflow_gar = elevation_outflow_gar)
#mid_rating_curve

# Define data for the lines
lines_data <- data.frame(
  x = c(2.5/3.28, 2.8/3.28, 0.4, 0.4),
  xend = c(2.5/3.28, 2.8/3.28, 2.5/3.28, 2.8/3.28),
  y = c(0, 0, elevation_outflow_gar[25], elevation_outflow_gar[28]),
  yend = c(elevation_outflow_gar[25], elevation_outflow_gar[28], elevation_outflow_gar[25], elevation_outflow_gar[28])
)

# Define data for the first ribbon (between the first two segments)
ribbon1_data <- data.frame(
  x = c(2.5/3.28, 2.8/3.28, 2.8/3.28, 2.5/3.28),
  y = c(0, 0, elevation_outflow_gar[28], elevation_outflow_gar[25])
)

# Define data for the second ribbon (between the second two segments)
ribbon2_data <- data.frame(
  x = c(0.4, 0.4, 2.8/3.28, 2.5/3.28),
  y = c(elevation_outflow_gar[25], elevation_outflow_gar[28], elevation_outflow_gar[28], elevation_outflow_gar[25])
)

# Create the plot
ggplot() +  
  # Add the lines
  geom_segment(data = lines_data, aes(x = x, xend = xend, y = y, yend = yend), color = "red") +
  # Add the first ribbon (shaded area between the first two segments)
  geom_polygon(data = ribbon1_data, aes(x = x, y = y), fill = "red", alpha = 0.2) +
  # Add the second ribbon (shaded area between the second two segments)
  geom_polygon(data = ribbon2_data, aes(x = x, y = y), fill = "red", alpha = 0.2) +
  geom_line(data = gar_rating_curve, aes(x = elevation_relationship_ft, y = elevation_outflow_gar), 
            color = "black", size = 1.5) + # Customize color and size of the line 
  # Set plot limits
  xlim(0, 5/3.28) +
  ylim(0, 8/35.3146667) +
    labs(title = "",
  ...
  theme_minimal() +
   theme(
    ...
    scale_x_continuous(limits = c(0.4, 0.89), expand = c(0, 0))  + # Set x-axis limits and remove extra space
    scale_y_continuous(limits = c(0, 0.44), expand = c(0, 0))  # Set y-axis limits and remove extra space
```
Midtown Fancy Rating Curve Plot
```{r}
library(ggplot2)

#elevation_relationship_ft <- elevation_relationship_ft * 3.28
#elevation_outflow_mid <- elevation_outflow_mid / 35.3146667
mid_rating_curve <- data.frame(elevation_relationship_ft = elevation_relationship_ft, elevation_outflow_mid = elevation_outflow_mid)

# Define data for the lines
lines_data <- data.frame(
  x = c(2.5/3.28, 2.8/3.28, 0.4, 0.4),
  xend = c(2.5/3.28, 2.8/3.28, 2.5/3.28, 2.8/3.28),
  y = c(0, 0, elevation_outflow_mid[25], elevation_outflow_mid[28]),
  yend = c(elevation_outflow_mid[25], elevation_outflow_mid[28], elevation_outflow_mid[25], elevation_outflow_mid[28])
)

# Define data for the first ribbon (between the first two segments)
ribbon1_data <- data.frame(
  x = c(2.5/3.28, 2.8/3.28, 2.8/3.28, 2.5/3.28),
  y = c(0, 0, elevation_outflow_mid[28], elevation_outflow_mid[25])
)

# Define data for the second ribbon (between the second two segments)
ribbon2_data <- data.frame(
  x = c(0.4, 0.4, 2.8/3.28, 2.5/3.28),
  y = c(elevation_outflow_mid[25], elevation_outflow_mid[28], elevation_outflow_mid[28], elevation_outflow_mid[25])
)
...
ggplot() +  
  ...
  # Add the second ribbon (shaded area between the second two segments)
  geom_polygon(data = ribbon2_data, aes(x = x, y = y), fill = "red", alpha = 0.2) +
  geom_line(data = mid_rating_curve, aes(x = elevation_relationship_ft, y = elevation_outflow_mid), 
            color = "black", size = 1.5) + # Customize color and size of the line 
  # Set plot limits
  xlim(0, 5/3.28) +
  ylim(0, 8/35.3146667) +
...
```{r}
plot(depth_ft_gar[27258:27656])
plot(depth_ft_mid[1294:1692])
```
